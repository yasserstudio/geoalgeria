...
> restent dans JSON/CSV, avec `geo_precision`/`geo_method` à `null`). Pour 79
...
valeur source non documentée `"1"`) ; `commune_code` est renseigné pour 79
...
leur `commune_code` canonique et 79 DAB lorsque le nom français unique de la
