...
الإحداثيات: 572 مُموقَعة (552 بدقّة `exact`، و20 بدقّة `approximate`).
...
> الأسماء والأنواع والإحداثيات من **المتعامل** (`geo_method: "operator_api"`). الإحداثيات بدقّة `geo_precision: "exact"` لـ552 نقطة بيع، وبدقّة `"approximate"` لـ20 نقطة: 19 إحداثيّتها المصدرية أقل من 3 خانات عشرية، ونقطة واحدة (`31-001`) وقع إحداثيّها في الواجهة داخل البحر فأُسنِدت إلى مركز بلديتها (`geo_method: "commune_centroid"`). الولاية دقيقة فعليًا (من النقطة المُعايَنة)؛ والبلدية أقرب مركز (بأفضل جهد). بما أنّ الواجهة تصنّف النقاط وفق تقسيم 48 ولاية القديم، يُعاد اشتقاق الولاية/البلدية من الإحداثيات إلى تقسيم 69 ولاية الحالي؛ وتُحفظ ولاية المتعامل المُعلنة في `operator_wilaya`.
