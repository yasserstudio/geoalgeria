...
الإحداثيات: 572 مُموقَعة (553 بدقّة `exact`، و19 بدقّة `approximate`).
...
> الأسماء والأنواع والإحداثيات من **المتعامل** (`geo_method: "operator_api"`). الإحداثيات بدقّة `geo_precision: "exact"` لـ553 نقطة بيع، وبدقّة `"approximate"` لـ19 نقطة إحداثيّتها المصدرية أقل من 3 خانات عشرية. الولاية دقيقة فعليًا (من النقطة المُعايَنة)؛ والبلدية أقرب مركز (بأفضل جهد). بما أنّ الواجهة تصنّف النقاط وفق تقسيم 48 ولاية القديم، يُعاد اشتقاق الولاية/البلدية من الإحداثيات إلى تقسيم 69 ولاية الحالي؛ وتُحفظ ولاية المتعامل المُعلنة في `operator_wilaya`.
