...
التي تتركها المصدر فارغة؛ `commune` و`daira` بالفرنسية (أحرف كبيرة، كما مُنشرة)؛ يحمل
`commune_code` رمز البلدية في 128 سجلاً تتطابق فيها التسمية الحالية أو الرسمية للديوان
الوطني للإحصائيات لسنة 2021 مع النقطة، ويكون `null` فيما عدا ذلك. للحصول على قائمة كاملة بتقسيمات الولايات والبلديات، اربط `wilaya_code`
بمجموعة بيانات [`geoalgeria`](https://www.npmjs.com/package/geoalgeria). `wilaya_code` مُكمَّل
بصفر على خانتين ويتبع نموذج الولايات الـ69 الحالي. وعندما يختلف عن قيمة الوزارة السابقة
للإصلاح، يحتفظ `source_wilaya_code` بقيمة المصدر. قيمة `geo_precision` هي `"exact"` لـ 2,244 سجلا و`"approximate"` لـ 90 سجلا
