...
| `any.whitespace` | `conservative` | `any` | Repeated and leading separators are typing, not naming: a separator that meets no open word adds nothing to the key, so the key is the words joined by one space and a partial last word can still complete. |
