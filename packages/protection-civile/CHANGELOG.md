...
- **Wilaya re-derived by point-in-polygon** against the 69 post-2026-reform
  boundaries, so units in the 11 new wilayas carry their correct code; the DGPC's
  pre-reform `cod_wilaya` is preserved in `refs.dgpc_wilaya`. Commune best-effort
  (Arabic name match, nearest-centroid fallback), stable `{wilaya}-{seq}` ids.
