...
| الخطوط الجوية المباشرة | **124** | خطوط اتجاهية مع المشغّل والحالة ومستوى الإثبات والمصدر |
| الخطوط الجوية المخطّطة | **18** | معلن عنها ولم تبدأ بعد؛ مجموعة منفصلة، وليست حالة |
...
كل طرف من أطراف الخطوط (طرفا كل قوس، بما في ذلك المطارات الأجنبية) يحمل اسما بثلاث لغات:
`name` (بالفرنسية؛ تسمية Wikidata الفرنسية للمطارات الأجنبية، وأسلوب ANAC للمطارات
الجزائرية) و`name_en` و`name_ar`، مطابقة عبر Wikidata برمز IATA ومدققة مقابل
الإحداثيات المرفقة. الخطوط تتغير موسميا، لذا يسجل `metadata.routes_as_of` تاريخ آخر تدقيق
للشبكة مقابل جداول الرحلات: لقطة مؤرخة، لا حقيقة دائمة.
