...
// 1,920 سجلا تحتوي على إحداثيات – الترتيب حسب المسافة، الخرائط، أو أقرب مركز في بضعة أسطر.
```

## ما يمكنك بناؤه

- **البحث عن أقرب مركز تكوين** – 1,920 سجلا محدد الإحداثيات، جاهزة للترتيب حسب المسافة.
- **أدلة التكوين المهني** – أسماء ثنائية اللغة، النوع، الطاقة الاستيعابية وبيانات الاتصال الكاملة.
- **الخرائط** – طبقة نقاط GeoJSON جاهزة للاستخدام لشبكة التكوين المهني (99% محدد الإحداثيات).
- **تخطيط الطاقة الاستيعابية** – الطاقة النظرية والفعلية، توفر الإقامة الداخلية والمساحة.
- **التحليل القطاعي** – 1,209 مؤسسة عمومية مقابل 723 خاصة عبر الولايات الحالية الـ69.
...
تغطي **الولايات الحالية الـ69**. يعتمد المصدر مخطط 58 ولاية السابق للإصلاح، لذلك يطابق
المولّد الإحداثيات والبلديات مع التقسيم الحالي وينسب 134 مؤسسة إلى الولاية الجديدة المنبثقة عن إصلاح
2026. ومن أصل 1,932 مؤسسة، هناك 1,920 مؤسسة محددة الإحداثيات
(99%) – `lat`/`lng` قيمتهما `null` للـ 12 المتبقية. من بين السجلات المحددة الإحداثيات،
1,375 تحمل النقطة المنشورة من takwin.dz (`geo_precision` `"exact"`)؛ أما البقية فالبوابة
تترك إحداثياتها فارغة، لذلك وُضع 510 منها على مركز بلديتها و35 على مركز ولايتها، وكلاهما
`"approximate"` ومُوضّح في `geo_method`.
...
data/
  ...
  geojson/                 # معالم GeoJSON (1,920 نقطة محددة الإحداثيات)
```
...
وفق مخطط الولايات الحالي الـ69، ويُشتق بمطابقة بلدية المصدر؛ `commune_code` حاليا دائما `null` في هذا المصدر (لا يوجد رمز ONS منشور
من طرف takwin.dz). `lat`/`lng`، وكذلك `geo_precision`/`geo_method` معهما، قيمتها `null`
للـ 12 سجلا غير المحددة الإحداثيات بعد؛ عند وجودها، `geo_precision` تكون
`"exact"` أو `"approximate"` و`geo_method` تسجل مصدر الإحداثية: `takwin` لنقطة منشورة،
و`commune` أو `wilaya` لمركز يحل محل إحداثية لم تملأها البوابة قط. `capacite`
...
المصدر يستخدم **مخطط 58 ولاية** (قبل الإصلاح)، ويطابق المولّد بلدياته مع التقسيم الحالي
ذي 69 ولاية. شغل `npm run fetch` لإعادة تشغيل الالتقاط المحفوظ للمصدر. الأسماء والأنواع
وبيانات الاتصال والطاقات الاستيعابية والإحداثيات كما نشرتها الوزارة.
