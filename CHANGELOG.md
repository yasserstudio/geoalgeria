...
- **Schools** (`@geoalgeria/ecoles`, new): 11,833 schools and kindergartens across all 69 wilayas, extracted from OpenStreetMap (ODbL) — classified by cycle (3,969 primaire, 2,378 moyen/CEM, 1,575 secondaire/lycée, 283 préscolaire; the rest `autre`), with bilingual French/Arabic names (8,643 named), a `sector` flag where the map signals it (313 public, 48 private), and commune/wilaya linkage by nearest-centroid join. Cycle is inferred from `isced:level` and the FR/AR name — a CEM names itself متوسطة/collège, a lycée ثانوية/lycée — with a bare "école"/"مدرسة" classified `primaire` by Algerian convention; 92% of named schools resolve to a specific cycle. Framed honestly as a partial OSM extract (~11.8k mapped against the ~28,000 of the national network), not an official registry.
