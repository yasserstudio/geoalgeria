...
how many departures that observation held, and the estimated duration derived
from SOGRAL's own segment figures per the boundary below. It ships as one
committed file in the app repository
(`apps/web/public/geodata/sogral-network.json`), generated by
`pnpm prepare-sogral-network`, carrying an observation date, a source sentence
...
| --- | --- | --- |
| Observed network: connection, departure count, estimated duration | Public `/departures` | `Observed on {date}.` (Algiers calendar day) plus the no-times line below |
...
primary action. The observation date is a calendar day with no clock time on
it: an hour beside a bus route reads as a departure time, which is the one
...
The observation date is mandatory. A departure **count** is publishable and is
labelled as observed (`12 observed departures`), because it describes what one
observation held, not what runs today. An individual departure is not
